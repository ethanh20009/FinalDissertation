...
def parse_args():
    ...
    parser.add_argument(
        "--initial", type=str, default="random", choices=["random", "circle"]
    )
...
class sparse_coord_init:
    def __init__(
        self, pred, gt, format="[bs x c x 2D]", quantile_interval=200, nodiff_thres=0.1
    ):
        if isinstance(pred, torch.Tensor):
            pred = pred.detach().cpu().numpy()
        if isinstance(gt, torch.Tensor):
    ...
    for i in range(num_paths):
        ...
        if seginit_cfg.type == "random":
            ...
            for j in range(num_segments):
                ...
                p1 = (
                    p0[0] + radius * npr.uniform(-0.5, 0.5),
                    p0[1] + radius * npr.uniform(-0.5, 0.5),
                )
                p2 = (
                    p1[0] + radius * npr.uniform(-0.5, 0.5),
                    p1[1] + radius * npr.uniform(-0.5, 0.5),
                )
                p3 = (
                    p2[0] + radius * npr.uniform(-0.5, 0.5),
        ...
        # circle points initialization
            ...
            points = get_bezier_circle(
                radius=radius, segments=num_segments, bias=center
            )
        ...
        if "gt" in kwargs:
            ...
            fill_color_init = list(kwargs["gt"][0, :, href, wref]) + [1.0]
...
def live_run(
    ...
    signature,  # List[str]
    config: str = "liveconfig/base.yaml",
    ...
    cfg.experiment_dir = osp.join(
        cfg.log_dir, "{}_{}".format(cfg.exid, "_".join(cfg.signature))
    )
    ...
    # Load target image if specified as path.
    gt = np.array(PIL.Image.open(cfg.target)) if type(cfg.target) == str else cfg.target
    print(f"Input image shape is: {gt.shape}")

    # Write target png to folder
    target_path = osp.join(cfg.experiment_dir, "target.png")
    ...
    if cfg.trainable.bg:
        ...
        para_bg = torch.tensor([1.0, 1.0, 1.0], requires_grad=True, device=device)
    else:
        if cfg.use_ycrcb:
            para_bg = torch.tensor(
                [219 / 255, 0, 0], requires_grad=False, device=device
            )
        else:
            para_bg = torch.tensor([1.0, 1.0, 1.0], requires_grad=False, device=device)
    ...
    for path_idx, pathn in enumerate(path_schedule):
        ...
        if cfg.save.init:
            filename = os.path.join(
                cfg.experiment_dir, "svg-init", "{}-init.svg".format(pathn_record_str)
            )
        ...
        for t in t_range:
            ...
            if cfg.save.video:
                ...
                if cfg.use_ycrcb:
                    imshow = (
                        ycrcb_conversion(img, format="[2D x 3]", reverse=True)
                        .detach()
                        .cpu()
                    )
                else:
            ...
            if cfg.save.loss:
                ...
                save_weighted_loss = (
                    save_weighted_loss - np.min(save_weighted_loss)
                ) / np.ptp(save_weighted_loss)
                ...
                filename = os.path.join(
                    cfg.experiment_dir,
                    "loss",
                    "{}-iter{}-mseloss.png".format(pathn_record_str, t),
                )
                check_and_create_dir(filename)
                ...
                filename = os.path.join(
                    cfg.experiment_dir,
                    "loss",
                    "{}-iter{}-sdfweight.png".format(pathn_record_str, t),
                )
                plt.savefig(filename, dpi=800)
                plt.close()
                ...
                filename = os.path.join(
                    cfg.experiment_dir,
                    "loss",
                    "{}-iter{}-weightedloss.png".format(pathn_record_str, t),
                )
                plt.savefig(filename, dpi=800)
                plt.close()
            ...
            #     loss = loss + loss_bis
            if (cfg.loss.xing_loss_weight is not None) and (
                cfg.loss.xing_loss_weight > 0
            ):
        ...
        if cfg.save.video:
            ...
            videoname = os.path.join(
                cfg.experiment_dir, "video-avi", "{}.avi".format(pathn_record_str)
            )
